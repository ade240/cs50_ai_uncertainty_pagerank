...
def sample_pagerank(corpus, damping_factor, n):
    ...
    # add that probaility to the distribution dictionary
    dist1[randpage] = 1/n
    # choose rest of samples by using transition model from the first page
    for i in range(n-1):
        # transtion model returns a probaility distribution
...
def iterate_pagerank(corpus, damping_factor):
    ...
    # repeat until no page probability changes more than .001
    while max(deltas.values()) > .001:
        for current_page in dist2:
            ...
            for page in corpus:
                ...
                if not corpus[page]:
                    # if page doesn't link to any other page, distribute probaility evenly
